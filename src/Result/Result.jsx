...
import { bus, train, flight } from "../data";
const SearchResults = () => {
  ...
  const findTransportDetails = () => {
    ...
    switch (formData.transportType) {
      ...
      case "train":
        if (
          train.source === formData.source &&
          train.destination === formData.destination
        ) {
          details = train;
        }
        break;
  ...
  return (
    <Box sx={{ padding: 3 }}>
      ...
      {transportDetails ? (
        <Box>
          ...
          {formData.transportType === "bus" && (
            <Box>
              ...
              <Typography>
                Stops: {transportDetails.stops.join(", ")}
              </Typography>
              <Typography>Fare: ${transportDetails.fare}</Typography>
              <Typography>
                Seats Available: {transportDetails.noOfSeatsAvailable}
              </Typography>
            </Box>
          )}
            ...
            <Box>
              ...
              {transportDetails.coaches.map((coach) => (
                <Typography key={coach.coachName}>
                  {coach.coachName} - Seats Available:{" "}
                  {coach.noOfSeatsAvailable}, Fare: ${coach.fare}
                </Typography>
              ))}
            </Box>
          )}
          {formData.transportType === "flight" && (
            <Box>
              ...
              <Typography>
                Flight Name: {transportDetails.flightName}
              </Typography>
              <Typography>
                Stops: {transportDetails.stops.join(", ")}
              </Typography>
              ...
              {transportDetails.categories.map((category) => (
                <Typography key={category.categoryName}>
                  {category.categoryName} - Seats Available:{" "}
                  {category.noOfSeatsAvailable}, Fare: ${category.fare}
                </Typography>
              ))}
            </Box>
          )}
        ...
        <Typography variant="body1">
          No transport details found for the selected route.
        </Typography>
      )}
    </Box>
  );
